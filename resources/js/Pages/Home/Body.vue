   ...
   <section class="bg-white py-8">
            ...
            <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
                            ...
                            <jet-dropdown>
                                ...
                                <template #content>
                                    <template v-if="categories">
                                        ...
                                        :class="!selectedCategory?'bg-yellow-300':''"
                                        @click="selectedCategory=''"
                                        >All products</p>
                                        <p 
                                        v-for="category in categories" :key="category.id" class="cursor-pointer block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition" 
                ...
                <template v-if="allProducts.length">
                    <div class="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col" v-for='product in allProducts' :key="product.id">
                        <inertia-link :href="route('show',product.id)">
                            <img class="hover:grow hover:shadow-lg w-full" :src="product.image_path">
                            <div class="pt-3 flex items-center justify-between">
                                <p class="">{{product.name}}</p>
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 fill-current text-gray-500 hover:text-black" viewBox="0 0 24 24"
                                    fill="currentColor">
                                    <path
                    ...
                    <pagination :links="links" class="mx-auto my-5"/>
                </template>
...
export default {
    ...
    computed:{
        filtredProducts(){
            return this.selectedCategory?this.products.filter(i=>i.categories.map(i=>i.id).includes(this.selectedCategory)):this.products;
        },
        allProducts(){
            return this.searchable?this.filtredProducts.filter(item=>item.name.toLowerCase().split(" ").includes(this.searchable.toLowerCase())):this.filtredProducts;
        }
    },
    methods:{
        getPrice(i){
